...
-- Выбираем данные для отчета: номер карты, код RFM и группу клиентов на основе комбинации RFM-кодов.
SELECT 
    card AS "Клиент", 
    CONCAT(Recency, Frequency, Monetary) AS RFM_code,
    CASE 
        WHEN CONCAT(Recency, Frequency, Monetary) = '111' THEN 'VIP-клиенты'
    ...
    END AS "Группа"
FROM RFM
ORDER BY 3 -- Сортируем по группе, чтобы упорядочить вывод в соответствии с категориями клиентов.
